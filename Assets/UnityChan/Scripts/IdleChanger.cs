	...
	public class IdleChanger : MonoBehaviour
	{
      ...
      //private float _seed = 0.0f;					// ランダム判定用シード
		...
		void OnGUI ()
		{
            if (isGUI)
            {
	            UnityEngine.GUI.Box(new Rect(Screen.width - 110, 10, 100, 90), "Change Motion");
                if (UnityEngine.GUI.Button(new Rect(Screen.width - 100, 40, 80, 20), "Next"))
                    anim.SetBool("Next", true);
                if (UnityEngine.GUI.Button(new Rect(Screen.width - 100, 70, 80, 20), "Back"))
                    anim.SetBool("Back", true);
            }
		}
